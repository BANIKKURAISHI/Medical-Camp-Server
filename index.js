...
async function run() {
  try {
   // await client.connect();
    ...
    const CampPaymentCollection = client
      .db("Medical")
      .collection("campPayment");
    const reviewCollection = client.db("Medical").collection("review");
    const teamCollection = client.db("Medical").collection("Team");
    const participantCollection = client.db("Medical").collection("participant");
    ////jwt
    ...
    app.patch("/update/:id", verifyToken,async (req, res) => {
      ...
      const result = await adminAddCollection.updateOne(
        query,
        document,
        options
      );
